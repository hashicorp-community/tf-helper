...
tfh_run_discard () {
  ...
  if [ -z "$run_ids" ]; then
    ...
    run_ids="$(printf "%s" "$list_resp" |
      jq -r '.data[] | select(.attributes.actions."is-discardable") |
        .id')"

    if [ -z "$run_ids" ]; then
      echoerr "unable to locate a discardable run"
      return 1
    fi
  fi
  ...
  for run_id in $run_ids; do
    url="$address/api/v2/runs/$run_id/actions/discard"
    if ! discard_resp="$(tfh_api_call -d "$payload" "$url")"; then
      echoerr "unable to discard run $run_id"
      return 1
    fi
